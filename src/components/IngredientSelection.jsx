    ...
    return (
        <div className="min-h-screen pt-32 pb-48 px-4 bg-[var(--charcoal)] relative">
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="flex overflow-x-auto no-scrollbar md:flex-wrap justify-start md:justify-center gap-3 md:gap-4 mb-10 md:mb-20 bg-white/5 p-2 rounded-[2rem] md:rounded-[3rem] backdrop-blur-md border border-white/10 max-w-4xl mx-auto scroll-smooth px-4">
                    {categories.map((cat) => (
                        <button
                            key={cat.id}
                            onClick={() => setActiveTab(cat.id)}
                            className={`flex items-center gap-2 md:gap-3 px-6 md:px-8 py-3 md:py-4 rounded-full uppercase tracking-[0.2em] font-black text-[8px] md:text-[10px] transition-all duration-500 whitespace-nowrap ${activeTab === cat.id
                        ...
                        {INGREDIENTS[activeTab].map((item, idx) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`relative dark-glass p-0 rounded-[2rem] md:rounded-[2.5rem] cursor-pointer group group overflow-hidden border-2 transition-all duration-500 ${isSelected
                                ...
                                >
                                    ...
                                    <div className="relative h-48 sm:h-64 w-full flex items-center justify-center p-6 md:p-8 bg-gradient-to-b from-white/5 to-transparent">
                                        ...
                                        <div className={`absolute top-4 right-4 md:top-6 md:right-6 w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isSelected ? 'bg-[var(--amber)] text-[var(--charcoal)] rotate-0' : 'bg-white/10 text-white/40 rotate-45 group-hover:rotate-0'
                                            ...
                                            {isSelected ? <Minus size={16} md:size={20} strokeWidth={3} /> : <Plus size={16} md:size={20} strokeWidth={3} />}
                                        </div>
                                    </div>

                                    {/* Content Info */}
                                    <div className="p-6 md:p-8 space-y-2 text-center">
                                        <h3 className="display-font text-xl md:text-2xl font-bold text-white group-hover:text-[var(--gold)] transition-colors">{item.name}</h3>
                                        <p className="text-[9px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.3em]">{item.desc}</p>
                                    </div>
            ...
            <AnimatePresence>
                {ingredients.length > 0 && (
                    <motion.div
                        ...
                        animate={{ y: 0 }}
                        exit={{ y: 200 }}
                        className="fixed bottom-6 md:bottom-10 left-0 right-0 z-50 px-4"
                    >
                        <div className="max-w-4xl mx-auto dark-glass rounded-[2.5rem] md:rounded-[3.5rem] p-4 md:p-6 border border-white/10 flex flex-col md:flex-row items-center gap-6 md:gap-10 justify-between">
                            <div className="flex items-center gap-4 md:gap-8 w-full md:w-auto justify-between md:justify-start px-4">
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2">
                                        <h4 className="text-[var(--gold)] text-[8px] md:text-[10px] font-bold uppercase tracking-[0.4em]">Reservoir</h4>
                                    ...
                                    <div className="flex -space-x-2 md:-space-x-4">
                                        {ingredients.map((ig, i) => (
                                            <motion.div
                                                key={ig.id}
                                                layoutId={`tray-${ig.id}`}
                                                className="w-10 h-10 md:w-14 md:h-14 rounded-full border-2 border-[var(--charcoal)] bg-white p-1 shadow-xl relative group z-10"
                                            >
                                        ...
                                        {Array.from({ length: 5 - ingredients.length }).map((_, i) => (
                                            <div key={i} className="w-10 h-10 md:w-14 md:h-14 rounded-full border-2 border-dashed border-white/10 bg-white/5 flex items-center justify-center text-white/10 text-[10px] font-bold">
                                                {ingredients.length + i + 1}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                <div className="h-10 md:h-12 w-px bg-white/10 hidden sm:block" />
                                <div className="text-[var(--gold)] flex items-end gap-1">
                                    <span className="text-3xl md:text-5xl font-black font-mono leading-none tracking-tighter">{ingredients.length}</span>
                                    <span className="text-xs md:text-sm opacity-30 font-bold mb-1">/5</span>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className="group relative overflow-hidden bg-white text-[var(--charcoal)] px-8 md:px-12 py-4 md:py-5 rounded-full font-black uppercase text-xs md:text-[10px] tracking-[0.2em] md:tracking-[0.3em] flex items-center gap-3 md:gap-4 hover:pr-16 transition-all duration-500 active:scale-95 shadow-[0_0_50px_rgba(255,255,255,0.2)] w-full md:w-auto justify-center"
                            >
